...
// Для неявного ДО массив изначально заполнен нулями
// нельзя построить неявное до для массива заполненого чем то изначально
...
struct SegTree {
  ...
  void check() {
    if (left == nullptr) left = new SegTree();
    if (right == nullptr) right = new SegTree();
  }
...
//  for (int i = 0; i < n; i++) {
//    std::cin >> a[i];
//    tree.update_in_point(0, n, i, a[i]);
//  }
  std::cin >> m;
  for (int i = 0; i < m; i++) {
    std::string com;
